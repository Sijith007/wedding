...
// Particles
	...
	$('.wed_image_bck').each(function(){
		...
		var altimage = $(this).attr('data-alt-image');
		...
		if(altimage){
			...
			$(_this).css('WebkitTransition', 'background-image 2s ease-in-out');
			$(_this).css('transition', 'background-image 2s ease-in-out');

			window.setInterval(function(){
				if(isAltImage){
					$(_this).css('background-image', 'url('+image+')');
				}else{
					$(_this).css('background-image', 'url('+altimage+')');
				}
		...
		if(altimage){
			window.setInterval(function(){
				if(isAltImage){
					...
					$(this).css('background-image', 'url('+altimage+')');
				}
				isAltImage = !isAltImage;
			}, 5000);
		}
	...
	var i = 0;	
		$(this).find('.wed_icon_box, .skill-bar-content, .wed_anim_box, .wed_bd, .wed_story_txt, .wed_portfolio_item_cont').each(function(){
			$(this).css('transition-delay','0.0s');
			i++;
		})
	})
